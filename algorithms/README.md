...
| `tape_query_table.hexa` (v1.1 P4) | CLI view: walk a `.tape`, find adjacent `@D` events sharing the same continuation key-set, render each run as a markdown table. Optional `--section=<label>` filter binds to a `# ── Section ──────` divider context. Pure-view layer (stdout only). Closes the round-trip: tables flattened to `@D` rows by P2 become readable tables again on demand. |
...
hexa algorithms/tape_query_table.hexa <tape.in> [--section=<label>]
```
